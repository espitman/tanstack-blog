...
import { getAccommodationsByProvinceFn } from '@/lib/accommodations/accommodation.server-functions'
...
export const Route = createFileRoute('/province/$name')({
  ...
  loader: async ({ params }) => {
    ...
    const accommodations = await getAccommodationsByProvinceFn({
      data: { provinceName, params: { pageSize, pageNumber: 1 } },
    })
    
    const hasMore = accommodations.length === pageSize
    return { accommodations, pageSize, hasMore, provinceName }
  },
})

function ProvinceAccommodations() {
  const { accommodations: initialAccommodations, pageSize, hasMore: initialHasMore, provinceName } = Route.useLoaderData()
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    getAccommodationsByProvinceFn({
      data: { provinceName, params: { pageSize, pageNumber: currentPage } },
    }).then((data) => {
      setAccommodations(data)
      setHasMore(data.length === pageSize)
      setIsLoading(false)
    })
  }, [currentPage, pageSize, provinceName, initialAccommodations])
  ...
  const displayProvinceName = accommodations.length > 0 
    ? accommodations[0].location.province 
    : provinceName
        .split('_')
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 via-white to-gray-50">
      <div className="container mx-auto px-4 py-12 max-w-7xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">اقامتگاه‌های {displayProvinceName}</h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-gray-600 mb-4">هیچ اقامتگاهی در {displayProvinceName} یافت نشد</p>
            <Link to="/accommodations">
